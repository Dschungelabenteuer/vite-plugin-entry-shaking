...
export function useDetails(props: DetailsProps, $toaster: Toaster) {
  ...
  const copyAbsolutePath = () => {
    ...
    $toaster.add({ type: 'success', message: 'Copied absolute path to clipboard!' });
  };

  const copyRelativePath = () => {
    navigator.clipboard.writeText(props.relativePath);
    $toaster.add({ type: 'success', message: 'Copied relative path to clipboard!' });
  };

  return { pathInputId, tabsContainerId, copyAbsolutePath, copyRelativePath };
}
