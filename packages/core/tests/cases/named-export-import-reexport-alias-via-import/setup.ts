 ...
 * ```
 */
export function testNamedExportImportReexportAliasViaImportSingle(middleTarget?: CaseTarget) {
 ...
 * ```
 */
export function testNamedExportImportReexportAliasViaImportMultiple(middleTarget?: CaseTarget) {
  describe('multiple re-exports', () => {
    ...
    it(`should work when target imports then re-exports modules using aliases. ${COMMON_EXPECTATION}`, async () => {
      ...
      const target = await createCaseTarget(targetName, 1);
      const { importPath, targetList } = setupCase(target, middleTarget);
      const input = `import { AliasedNamedExportOne, AliasedNamedExportTwo } from '${importPath}';\n`;
...
export function testNamedExportImportReexportAliasViaImport(middleTarget?: CaseTarget) {
  testNamedExportImportReexportAliasViaImportSingle(middleTarget);
  testNamedExportImportReexportAliasViaImportMultiple(middleTarget);
}
