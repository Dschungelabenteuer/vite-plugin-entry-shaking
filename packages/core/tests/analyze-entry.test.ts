...
describe.each([true, false])('with or without new line characters', newLineChar => {
  const createStatementVariant = (srcString: string) => newLineChar ? srcString.replaceAll(' ', '\n') : srcString;
  ...
  describe('parseImportStatement', () => {
    describe('aggregated export statements', () => {
      it('should correctly parse aggregated export statement', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`export { UserId, UserName } from '@model/user';`),
        );
      ...
      it('should correctly parse aggregated export statement with alias', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`export { UserId as UserIdentifier, UserName } from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: [createStatementVariant('UserId as UserIdentifier'), 'UserName'],
          defaultImport: null,
        });
      });
      ...
      it('should correctly parse aggregated export statement with only aliased default export', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`export { default as User } from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: [],
          defaultImport: 'User',
      ...
      it('should correctly parse aggregated export statement with aliased default export', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`export { default as User, UserName } from '@model/user';`),
        );
        expect(parsed).toMatchObject({
    ...
    describe('import statements', () => {
      it('should correctly parse import statement when only importing named exports', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`import { UserId, UserName } from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: ['UserId', 'UserName'],
          defaultImport: null,
      ...
      it('should correctly parse import statement when only importing default export', () => {
        const parsed = EntryAnalyzer.parseImportStatement(createStatementVariant(`import User from '@model/user';`));
        expect(parsed).toMatchObject({
          namedImports: [],
          defaultImport: 'User',
        });
      ...
      it('should correctly parse import statement when importing default export then named exports', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`import User, { UserId, UserName } from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: ['UserId', 'UserName'],
          defaultImport: 'User',
      ...
      it('should correctly parse import statement when importing named exports then default export', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`import { UserId, UserName }, User from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: ['UserId', 'UserName'],
          defaultImport: 'User',
      ...
      it('should correctly parse import statement when importing named exports with aliases', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`import { UserId as UserIdentifier, UserName }, User from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: [createStatementVariant('UserId as UserIdentifier'), 'UserName'],
          defaultImport: 'User',
      ...
      it('should correctly parse import statement when importing default export with alias', () => {
        const parsed = EntryAnalyzer.parseImportStatement(
          createStatementVariant(`import { default as User, UserName } from '@model/user';`),
        );
        expect(parsed).toMatchObject({
          namedImports: ['UserName'],
          defaultImport: 'User',
  ...
  describe('analyzeEntryImport', () => {
    ...
    describe('aggregated export statements', () => {
      it('should feed the "analyzedImports" map if this is an aggregated export', () => {
        const output = run(createStatementVariant(`export { UserId } from '${path}'`));
      ...
      it('should feed the "analyzedImports" map if this is an aggregated default export with alias', () => {
        const output = run(createStatementVariant(`export { default as User } from '${path}'`));
      ...
      it('should feed the "analyzedImports" map if this is an aggregated export with alias', () => {
        const output = run(createStatementVariant(`export { UserId as UserIdentifier } from '${path}'`));
    ...
    describe('import statements', () => {
      it('should feed the "analyzedImports" map if it imports named exports', () => {
        ...
        const output = run(createStatementVariant(`import { ${namedImport} } from "${path}"`));
      ...
      it('should feed the "analyzedImports" map if it imports a default export with an alias', () => {
        const importAlias = `MyUser`;
        const output = run(createStatementVariant(`import { default as ${importAlias} } from "${path}"`));
